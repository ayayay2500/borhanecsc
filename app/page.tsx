...
export default function Home() {
  ...
  // وظيفة استرداد الكود من السيرفر
  const handleRedeemCode = async () => {
    if (!giftCode.trim() || isSubmitting || !user) return
    ...
    try {
      ...
      if (data.success) {
        ...
        window.Telegram?.WebApp.showAlert(`✅ تم الاسترداد بنجاح! حصلت على ${data.amount} XP`)
        setShowGiftModal(false)
        setGiftCode('')
      } else {
        window.Telegram?.WebApp.showAlert(`❌ فشل: ${data.message || 'كود غير صالح'}`)
      }
    } catch (e) {
      window.Telegram?.WebApp.showAlert("❌ حدث خطأ في الاتصال بالسيرفر")
    } finally {
        ...
        <div className="modal-overlay">
          <div className="gift-modal">
            <div className="modal-header">
              <h3>استرداد كود هدايا 🎁</h3>
              <p>أدخل الكود للحصول على نقاط فورية</p>
            </div>
            
            <input 
              type="text" 
              placeholder="أدخل الكود هنا (مثلاً: GIFT50)" 
              value={giftCode}
            ...
            <div className="modal-actions">
              ...
              >
                {isSubmitting ? 'جاري التحقق...' : 'استرداد الرصيد'}
              </button>
              <button className="close-modal-btn" onClick={() => setShowGiftModal(false)}>
                إلغاء
              </button>
            </div>
          </div>
